...
def get_display_name(model_id: str, fallback_to_id: bool = True) -> str:
    """
    ...
    # Try mapping after stripping version/throughput suffix
    base_id = _strip_suffixes(model_id)
    if base_id in MODEL_NAME_MAP:
        suffix = _get_suffix_label(model_id)
        return f"{MODEL_NAME_MAP[base_id]}{suffix}"

    # Return model_id as-is if no mapping found
    if fallback_to_id:
        return model_id

    # If fallback_to_id is False, try parsing the model ID
    return _parse_model_id(model_id)


def _strip_suffixes(model_id: str) -> str:
    """Strip version and throughput suffixes from model ID."""
    # Remove throughput suffix like :0:28k, :0:200k
    result = re.sub(r':\d+:\d+k?$', '', model_id)
    # Handle case with only :0 version suffix
    if result == model_id:
        result = re.sub(r':\d+$', ':0', model_id)
    return result


def _get_suffix_label(model_id: str) -> str:
    """Convert throughput suffix to label."""
...
def _parse_model_id(model_id: str) -> str:
    """
    Parse model ID to generate human-readable name.

    Example: anthropic.claude-3-5-sonnet-20240620-v1:0 → Claude 3.5 Sonnet
    """
